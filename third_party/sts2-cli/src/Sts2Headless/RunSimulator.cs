...
public class RunSimulator
{
    ...
    // restore_combat can replay a long action prefix before the caller needs a
    // serialized observation.  In that path, building CombatPlayState after
    // every intermediate action is pure overhead (and is especially expensive
    // because it resolves every card preview against every live enemy).
    // Keep this scoped and opt-in so ordinary protocol commands retain their
    // existing, fully projected responses.
    private int _compactDecisionProjectionDepth;
        ...
        IReadOnlyList<string>? badges = null)
    {
        try
        {
            ...
            if (badges != null && _runState.BadgeModels is List<BadgeModel> badgeModels)
            {
                badgeModels.Clear();
                foreach (var rawId in badges)
                {
                    var entry = rawId.StartsWith("BADGE.", StringComparison.Ordinal)
                        ? rawId["BADGE.".Length..]
                        : rawId;
                    var model = ModelDb.GetById<BadgeModel>(new ModelId("BADGE", entry));
                    if (model == null) return Error($"Unknown badge: {rawId}");
                    badgeModels.Add(model);
                }
                Log($"Applied badges: {string.Join(",", badges)}");
            }
            ...
            // Initial setup can request a card or bundle choice before the
            // first act is entered. Register these hooks before Launch so the
            // decision remains externally visible.
        ...
        try
        {
            ...
            if (args.TryGetValue("deck", out var deckEl))
            {
                ...
                foreach (var cEl in deckEl.EnumerateArray())
                {
                    string? id;
                    int upgradeLevel = 0;
                    if (cEl.ValueKind == System.Text.Json.JsonValueKind.String)
                    {
                        id = cEl.GetString();
                    }
                    else if (cEl.ValueKind == System.Text.Json.JsonValueKind.Object)
                    {
                        id = cEl.TryGetProperty("id", out var idEl) ? idEl.GetString() : null;
                        if (cEl.TryGetProperty("upgrade_level", out var upgradeEl)
                            && upgradeEl.ValueKind == System.Text.Json.JsonValueKind.Number)
                            upgradeLevel = Math.Max(0, upgradeEl.GetInt32());
                    }
                    else
                    {
                        continue;
                    ...
                    if (canonical != null)
                    {
                        ...
                        for (int level = 0;
                             level < upgradeLevel && level < card.MaxUpgradeLevel;
                             level++)
                        {
    ...
    public Dictionary<string, object?> EnterRoom(
        ...
        string? encounter,
        string? eventId,
        bool compactDecisionProjection = false)
    {
        return WithDecisionProjection(
            compactDecisionProjection,
            () => EnterRoomCore(roomType, encounter, eventId));
    }

    private Dictionary<string, object?> EnterRoomCore(string roomType, string? encounter, string? eventId)
    {
        try
        {
            if (_runState == null) return Error("No run in progress");
    ...
    {
        internal PreparedSave(string json, byte[] packet)
        {
            Json = json;
            Packet = packet;
        }
        ...
        internal SerializableRun CreateIsolatedSave()
        {
    ...
    private static byte[] SerializePreparedSave(SerializableRun save)
    {
        ...
        out string error)
    {
        ...
        try
        {
            ...
            {
                error = $"Save schema mismatch: {schemaError}";
                return false;
            }
            ...
            {
                error = $"Failed to parse save file: {readResult.Status} {readResult.ErrorMessage}";
                return false;
            }

            prepared = new PreparedSave(
                saveJson,
                SerializePreparedSave(readResult.SaveData));
            return true;
        }
        catch (Exception ex)
        {
            error = $"PrepareSave failed: {ex.GetType().Name}: {ex.Message}";
            return false;
        }
    }
    ...
    {
        if (!TryPrepareSave(saveJson, out var prepared, out var error) || prepared == null)
            return Error(error);
        return LoadPreparedSave(prepared, lang);
    }

    public Dictionary<string, object?> LoadPreparedSave(
        ...
        string lang = "en")
    {
        try
        {
            _loc.Lang = lang;
            EnsureModelDbInitialized();
            Log("Loading prepared save...");

            // RunState.FromSerializable and SetUpSavedSingleplayer retain and
            // mutate nested save objects.  Reusing the parsed SerializableRun
            // directly leaks floor/RNG/container mutations across simulations.
            // The packet is prepared once, then deserialized into an isolated
            // object graph for every restore without reparsing JSON.
            var save = prepared.CreateIsolatedSave();
            _ascension = save.Ascension;
            ...
            var shouldResumeInitialNeow = IsInitialNeowSave(prepared.Json);
        ...
        {
            return ErrorWithTrace("LoadPreparedSave failed", ex);
        }
    }
        ...
        bool compactDecisionProjection = false)
    {
        return WithDecisionProjection(
            compactDecisionProjection,
            () => ExecuteActionCore(action, args));
    }

    private Dictionary<string, object?> ExecuteActionCore(
        string action,
        Dictionary<string, object?>? args)
    {
        try
    ...
    private Dictionary<string, object?> DoMapSelect(Player player, Dictionary<string, object?>? args)
    {
        ...
        // TestMode's boss-map shortcut advances directly to the post-boss
        // reward flow instead of starting the encounter.  That is convenient
        // for the game's map tests, but it makes a headless full-run replay
        // silently skip every act boss.  Preserve the map/floor bookkeeping
        // and enter the act's real boss encounter explicitly.
        var bossPoint = _runState?.Map?.BossMapPoint;
        var selectedPoint = _runState?.Map?.GetPoint(coord);
        var isBossCoord = (bossPoint != null
                && bossPoint.coord.col == coord.col
                && bossPoint.coord.row == coord.row)
            || selectedPoint?.PointType == MapPointType.Boss;
        if (isBossCoord)
        {
            if (_runState == null)
                return Error("No run in progress");
            var bossEncounter = _runState.Act?.BossEncounter;
            if (bossEncounter == null)
                return Error("Current act has no boss encounter");

            var floorBefore = _runState.ActFloor;
            Log($"Entering boss encounter {bossEncounter.Id.Entry} at ({col},{row})");
            RunManager.Instance.EnterMapCoordDebug(
                coord,
                RoomType.Boss,
                bossPoint!.PointType,
                bossEncounter.ToMutable(),
                showTransition: false).GetAwaiter().GetResult();
            if (_runState.ActFloor == floorBefore)
                _runState.ActFloor = floorBefore + 1;
            _syncCtx.Pump();
            WaitForActionExecutor();
            return DetectDecisionPoint();
        }
    ...
    private Dictionary<string, object?> DoEndTurn(Player player)
    {
        ...
        try
        {
            ...
            if (CombatManager.Instance.IsInProgress && !IsPlayPhase() && !player.Creature.IsDead)
            {
                ...
                {
                    _syncCtx.Pump();
                    // Enemy moves may intentionally pause the turn on an interactive
                    // card choice (Knowledge Demon: Curse of Knowledge).  That is a
                    // decision boundary, not a stuck enemy turn.
        ...
        // Surface selections opened by an enemy move before the deadlock recovery
        // path cancels the still-valid enemy-turn continuation.
        if (_cardSelector.HasPending || _cardSelector.HasPendingReward || _pendingBundles != null)
            return DetectDecisionPoint();
    ...
    /// <summary>
    /// Return the current externally visible decision state without advancing the run.
    /// This small protocol hook is used by snapshot/restore verification and simulator
    /// benchmarks; it intentionally reuses the same serializer as action responses.
    /// </summary>
    ...
    private Dictionary<string, object?> WithDecisionProjection(
        bool compact,
        Func<Dictionary<string, object?>> operation)
    {
        if (!compact)
            return operation();

        _compactDecisionProjectionDepth++;
        try
        {
            return operation();
        }
    ...
    private Dictionary<string, object?> ProjectCombatDecision(Player player)
    {
        if (_compactDecisionProjectionDepth <= 0)
            return CombatPlayState(player);

        // CombatPlayState normally maintains this terminal-reporting fallback.
        // Preserve that side effect even when its expensive visible projection
        // is skipped for an intermediate replay action.
    ...
    private Dictionary<string, object?> DetectDecisionPoint()
    {
        ...
        if (room is CombatRoom combatRoom)
        {
            ...
            {
                return ProjectCombatDecision(player);
            }
            ...
            for (int i = 0; i < 20; i++)
            {
                _syncCtx.Pump();
                Thread.Sleep(5);
                if (IsPlayPhase()) return ProjectCombatDecision(player);
    ...
    private Dictionary<string, object?> CombatPlayState(Player player)
    {
        ...
        var enemies = combatState?.Enemies?
            ...
            .Select((e, i) =>
            {
                ...
                {
                    ["index"] = i,
                    ["id"] = e.Monster?.Id.ToString() ?? "MONSTER.UNKNOWN",
                    ["combat_id"] = i.ToString(),
        ...
        var result = new Dictionary<string, object?>
        {
            ...
            {
                ["draw"] = CombatPileSummary(pcs?.DrawPile?.Cards),
                ["discard"] = CombatPileSummary(pcs?.DiscardPile?.Cards),
                ["exhaust"] = CombatPileSummary(pcs?.ExhaustPile?.Cards),
            },
        };
    ...
    private Dictionary<string, object?> ShopState(MerchantRoom merchantRoom, Player player)
    {
        ...
        var cards = inv.CharacterCardEntries.Concat(inv.ColorlessCardEntries)
            .Select((e, i) =>
            {
                ...
                {
                    ["index"] = i,
                    ["id"] = card?.Id.ToString() ?? "CARD.UNKNOWN",
        ...
        {
            ["index"] = i,
            ["id"] = e.Model?.Id.ToString() ?? "RELIC.UNKNOWN",
        ...
        {
            ["index"] = i,
            ["id"] = e.Model?.Id.ToString() ?? "POTION.UNKNOWN",
    ...
    private Dictionary<string, object?> PlayerSummary(Player player)
    {
        return new Dictionary<string, object?>
        {
            ["id"] = player.Character?.Id.ToString() ?? "CHARACTER.IRONCLAD",
            ...
            ["gold"] = player.Gold,
            ["relics"] = player.Relics?.Select((r, i) =>
            {
    ...
    private Dictionary<string, object?> RunContext()
    {
        ...
        var ctx = new Dictionary<string, object?>
        {
            ...
            ["badges"] = _runState.BadgeModels.Select(model => model.Id.ToString()).ToList(),
        };
    ...
    private static List<Dictionary<string, object?>> CombatPileSummary(IEnumerable<CardModel>? cards)
    {
        if (cards == null) return new();
        return cards
            .Where(card => card != null)
            .GroupBy(card => new { Id = card.Id.ToString(), Upgraded = card.IsUpgraded })
            ...
            .Select(group =>
            {
                ...
                var result = new Dictionary<string, object?>
                {
                    ...
                    ["upgrade_level"] = card.IsUpgraded ? 1 : 0,
                    ["stats"] = stats.Count > 0 ? stats : null,
                };
    ...
    private static void EnsureModelDbInitialized()
    {
        ...
        // v0.107.1 refuses model reflection until ModManager has reached a
        // terminal initialization state. Headless runs intentionally load no
        // mods, so mark discovery as skipped before RunState.CreateForTest.
        if (ModManager.State == ModManagerState.None)
        {
            typeof(ModManager).GetField("<State>k__BackingField",
        ...
        // Crusher and Rocket (the Kaiser Crab boss parts) resolve a scene-only
        // background node at the end of AfterAddedToRoom.  Keep their gameplay
        // powers but replace that final visual-only step in headless mode.
        PatchKaiserCrabSetup();
    ...
    private static void PatchKaiserCrabSetup()
    {
        try
        {
            var harmony = new Harmony("sts2headless.kaisercrabsetup");
            var assembly = typeof(CombatManager).Assembly;
            foreach (var (typeName, patchName) in new[]
            {
                ("MegaCrit.Sts2.Core.Models.Monsters.Crusher", nameof(YieldPatches.CrusherAfterAddedPrefix)),
                ("MegaCrit.Sts2.Core.Models.Monsters.Rocket", nameof(YieldPatches.RocketAfterAddedPrefix)),
            })
            {
            ...
            var hpChangedPrefix = typeof(YieldPatches).GetMethod(
                nameof(YieldPatches.KaiserHpChangedPrefix),
                System.Reflection.BindingFlags.Static | System.Reflection.BindingFlags.Public);
            ...
            })
            {
                var method = assembly.GetType(typeName)?.GetMethod(
                    ...
                    System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.Public);
                if (method != null && hpChangedPrefix != null)
                    harmony.Patch(method, new HarmonyMethod(hpChangedPrefix));
            }

            foreach (var (typeName, patchName, moveNames) in new[]
            {
                (
                    "MegaCrit.Sts2.Core.Models.Monsters.Crusher",
                    nameof(YieldPatches.CrusherMovePrefix),
                    new[] { "ThrashMove", "BugStingMove", "AdaptMove", "EnlargingStrikeMove", "GuardedStrikeMove" }
                ),
                (
                    "MegaCrit.Sts2.Core.Models.Monsters.Rocket",
                    nameof(YieldPatches.RocketMovePrefix),
                    new[] { "TargetingReticleMove", "PrecisionBeamMove", "ChargeUpMove", "LaserMove", "RechargeMove" }
                ),
            })
            {
                var monsterType = assembly.GetType(typeName);
                var prefix = typeof(YieldPatches).GetMethod(
                    patchName,
                    System.Reflection.BindingFlags.Static | System.Reflection.BindingFlags.Public);
                if (monsterType == null || prefix == null) continue;
            ...
            Console.Error.WriteLine("[INFO] Patched Kaiser Crab headless setup and visual-only move preambles");
        }
        catch (Exception ex)
        {
            Console.Error.WriteLine($"[WARN] Failed to patch Kaiser Crab setup: {ex.Message}");
        }
    }
    ...
    internal static class YieldPatches
    {
        ...
        public static bool CrusherAfterAddedPrefix(
            MegaCrit.Sts2.Core.Models.Monsters.Crusher __instance,
            ref Task __result)
        {
            __result = CrusherAfterAddedSafe(__instance);
            return false;
        }

        public static bool KaiserHpChangedPrefix(ref Task __result)
        {
            // Crusher/Rocket only use this hook to play a background hurt
            // animation. Damage has already been applied before the hook.
            __result = Task.CompletedTask;
            return false;
        }

        private static async Task CrusherAfterAddedSafe(
            MegaCrit.Sts2.Core.Models.Monsters.Crusher monster)
        {
            var creature = monster.Creature;
            await PowerCmd.Apply<BackAttackLeftPower>(
                new BlockingPlayerChoiceContext(), creature, decimal.One, creature, null, false);
            await PowerCmd.Apply<CrabRagePower>(
                new BlockingPlayerChoiceContext(), creature, decimal.One, creature, null, false);
        }

        public static bool RocketAfterAddedPrefix(
            MegaCrit.Sts2.Core.Models.Monsters.Rocket __instance,
            ref Task __result)
        {
            __result = RocketAfterAddedSafe(__instance);
            return false;
        }

        private static async Task RocketAfterAddedSafe(
            MegaCrit.Sts2.Core.Models.Monsters.Rocket monster)
        {
            var creature = monster.Creature;
            await PowerCmd.Apply<SurroundedPower>(
                new BlockingPlayerChoiceContext(),
                monster.CombatState.GetOpponentsOf(creature),
                decimal.One,
                creature,
                null,
                false);
            await PowerCmd.Apply<BackAttackRightPower>(
                new BlockingPlayerChoiceContext(), creature, decimal.One, creature, null, false);
            await PowerCmd.Apply<CrabRagePower>(
                new BlockingPlayerChoiceContext(), creature, decimal.One, creature, null, false);
        }

        public static bool CrusherMovePrefix(
            MegaCrit.Sts2.Core.Models.Monsters.Crusher __instance,
            IReadOnlyList<Creature> targets,
            MethodBase __originalMethod,
            ref Task __result)
        {
            __result = CrusherMoveSafe(__instance, targets, __originalMethod.Name);
            return false;
        }

        private static async Task CrusherMoveSafe(
            MegaCrit.Sts2.Core.Models.Monsters.Crusher monster,
            IReadOnlyList<Creature> targets,
            string moveName)
        {
            ...
            {
                case "ThrashMove":
                    await DamageCmd.Attack(ReadIntProperty(monster, "ThrashDamage"))
                        .FromMonster(monster).Execute(ctx);
                    break;
                case "BugStingMove":
                    await DamageCmd.Attack(ReadIntProperty(monster, "BugStingDamage"))
                        .WithHitCount(ReadIntProperty(monster, "BugStingTimes"))
                        .FromMonster(monster).Execute(ctx);
                    await PowerCmd.Apply<WeakPower>(ctx, targets, 2m, monster.Creature, null, false);
                    await PowerCmd.Apply<FrailPower>(ctx, targets, 2m, monster.Creature, null, false);
                    break;
                case "AdaptMove":
                    await PowerCmd.Apply<StrengthPower>(
                        ctx,
                        monster.Creature,
                        ReadIntProperty(monster, "AdaptStrengthGain"),
                        monster.Creature,
                        null,
                        false);
                    break;
                case "EnlargingStrikeMove":
                    await DamageCmd.Attack(ReadIntProperty(monster, "EnlargingStrikeDamage"))
                        .FromMonster(monster).Execute(ctx);
                    break;
                case "GuardedStrikeMove":
                    await DamageCmd.Attack(ReadIntProperty(monster, "GuardedStrikeDamage"))
                        .FromMonster(monster).Execute(ctx);
                    ...
                    break;
                default:
                    throw new InvalidOperationException($"Unsupported Crusher move: {moveName}");
            }
        }

        public static bool RocketMovePrefix(
            MegaCrit.Sts2.Core.Models.Monsters.Rocket __instance,
            IReadOnlyList<Creature> targets,
            MethodBase __originalMethod,
            ref Task __result)
        {
            __result = RocketMoveSafe(__instance, __originalMethod.Name);
            return false;
        }

        private static async Task RocketMoveSafe(
            MegaCrit.Sts2.Core.Models.Monsters.Rocket monster,
            string moveName)
        {
            var ctx = new BlockingPlayerChoiceContext();
            switch (moveName)
            {
                case "TargetingReticleMove":
                    await DamageCmd.Attack(ReadIntProperty(monster, "TargetingReticleDamage"))
                        .FromMonster(monster).Execute(ctx);
                    break;
                case "PrecisionBeamMove":
                    await DamageCmd.Attack(ReadIntProperty(monster, "PrecisionBeamDamage"))
                        .FromMonster(monster).Execute(ctx);
                    break;
                case "ChargeUpMove":
                    await PowerCmd.Apply<StrengthPower>(
                        ctx,
                        monster.Creature,
                        ReadIntProperty(monster, "ChargeUpStrengthGain"),
                        monster.Creature,
                        null,
                        false);
                    break;
                case "LaserMove":
                    await DamageCmd.Attack(ReadIntProperty(monster, "LaserDamage"))
                        .FromMonster(monster).Execute(ctx);
                    break;
                case "RechargeMove":
                    // The original move only restores the arm animation/audio.
                    break;
                default:
                    throw new InvalidOperationException($"Unsupported Rocket move: {moveName}");
            }
        }
        ...
        private static int ReadIntProperty(object instance, string propertyName)
        {
            ...
            if (property?.GetValue(instance) is not object value)
                throw new InvalidOperationException($"Missing numeric property {instance.GetType().Name}.{propertyName}");
    ...
    public void CleanUp()
    {
        try
        {
            ...
            CardSelectCmd.Reset();
            // CardSelectCmd.Reset installs the normal UI selector. Headless
            // reloads need an empty selector stack before UseSelector can bind
            // this simulator's selector again.
            var selectorStack = typeof(CardSelectCmd)
                .GetField("_selectorStack", BindingFlags.Static | BindingFlags.NonPublic)
                ?.GetValue(null);
            selectorStack?.GetType().GetMethod("Clear")?.Invoke(selectorStack, null);
            _turnStarted.Reset();
            _combatEnded.Reset();
            _runState = null;
        }
