...
class TestTurnFlow:
    def test_kaiser_crab_enemy_turns_work_without_scene_background(self, game):
        """Kaiser Crab gameplay must not depend on its visual background node."""
        state = game.start(seed="tf-kaiser-crab")
        game.skip_neow(state)
        game.set_player(
            hp=999,
            max_hp=999,
            deck=["DEFEND_IRONCLAD"] * 10,
            relics=["MERCURY_HOURGLASS"],
        )
        state = game.enter_room("combat", encounter="KAISER_CRAB_BOSS")

        assert state["decision"] == "combat_play"
        assert {enemy["name"] for enemy in state["enemies"]} == {"Crusher", "Rocket"}
        assert all(enemy["hp"] < enemy["max_hp"] for enemy in state["enemies"])

        initial_hp = state["player"]["hp"]
        for expected_round in range(2, 8):
            state = game.act("end_turn")
            assert state["decision"] == "combat_play"
            assert state["round"] == expected_round

        assert 0 < state["player"]["hp"] < initial_hp

    def test_enemy_turn_card_selection_is_exposed(self, game):
        """Knowledge Demon's Curse of Knowledge must pause for the two-card choice."""
        state = game.start(seed="tf-knowledge-select")
        game.skip_neow(state)
        game.set_player(hp=999, max_hp=999, deck=["DEFEND_IRONCLAD"] * 5)
        ...
        assert [card["id"] for card in state["cards"]] == [
            "CARD.DISINTEGRATION",
            "CARD.MIND_ROT",
        ]
